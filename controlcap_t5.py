...
class ControlCapT5(Blip2T5):
    ...
    def roi_align(self, image_embeds, samples, num_clusters=10):
        # Separate CLS token and spatial embeddings
        spatio_image_embeds = image_embeds[:, 1:]
        
        # Perform one-pass clustering on the spatial embeddings
        b, hw, c = spatio_image_embeds.shape
        spatio_image_embeds_flat = spatio_image_embeds.reshape(-1, c).cpu().detach().numpy()
        
        kmeans = KMeans(n_clusters=num_clusters, random_state=0).fit(spatio_image_embeds_flat)
        cluster_centers = torch.tensor(kmeans.cluster_centers_).to(image_embeds.device)
        
        # Replace original spatial embeddings with cluster centers
        cluster_assignments = torch.tensor(kmeans.labels_).to(image_embeds.device)
        spatio_image_embeds = cluster_centers[cluster_assignments].reshape(b, hw, c)
        
        # Reshape spatial embeddings to 2D image-like structure
    ...
    def prepare_control_words(self, samples, tag_logits):
        ...
        # Assume this MLP is defined as part of the class
        self.token_mlp = nn.Sequential(
            nn.Linear(self.t5_model.model_dim, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 512),
            nn.ReLU(),
            nn.Linear(512, self.t5_model.model_dim)
        )
        ...
        if self.training:
            for bz_idx, cap in enumerate(samples["caps"]):
                try:
                    ...
                    token_embeddings = self.t5_model.encoder.embed_tokens(
                        self.t5_tokenizer(tokens, return_tensors="pt", padding=True).input_ids.to(self.device)
                    )
                    
                    # Pass token embeddings through MLP
                    mlp_output = self.token_mlp(token_embeddings)
    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            # Add self-attention process
            query_hidden_states = query_output.last_hidden_state


            # Create a self-attention mask (assuming all tokens are valid)
            attention_mask = torch.ones(query_hidden_states.size()[:2], dtype=torch.long).to(image.device)

            # Self-attention layer
            self_attention_layer = nn.MultiheadAttention(
                embed_dim=query_hidden_states.size(-1),
                num_heads=8,  # You can adjust the number of heads
                batch_first=True
            ).to(image.device)

            # Perform self-attention
            query_output, _ = self_attention_layer(
                query_hidden_states,
                query_hidden_states,
                query_hidden_states,
            ...
            # If you need to maintain the same structure as the original query_output
            query_output = types.SimpleNamespace(last_hidden_state=query_output)
